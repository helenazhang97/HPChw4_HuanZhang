...
int main(int argc, char * argv[]){
  ...
  N=64;//the number of points in each dimension is as a power of 2 such that it can be easily split into sieces for 
  //2 x 2, 4 x 4, 8 x 8 etc processors, namely p
  // and sqrt(p) should be a power of 2.
  max_iters=10000;
  int dimp=int(sqrt(p));
  printf("processor per dimension is sqrt(p) = %d",dimp);
